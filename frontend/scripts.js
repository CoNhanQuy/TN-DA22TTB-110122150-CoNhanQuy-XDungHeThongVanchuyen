...
function escapeHtml(str) {
    // Cách encode an toàn, tránh bị "revert" bởi formatter/replace khác
...
function formatDateTime(value) {
    if (!value) return '';
    // value từ MySQL timestamp thường dạng: "YYYY-MM-DD HH:mm:ss"
...
async function trackOrder() {
    ...
    try {
        ...
        const res = await fetch('/DATN/backend/index.php?action=track', {
            method: 'POST',
            body: form,
        });

        // Nếu API trả về HTML/warning (không phải JSON), res.json() sẽ throw -> ta bắt để hiện lỗi rõ hơn
        let data;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const orderCCCDInput = document.getElementById('orderCCCD');

    [orderCodeInput, orderPhoneInput, orderCCCDInput].forEach((el) => {
        if (!el) return;
