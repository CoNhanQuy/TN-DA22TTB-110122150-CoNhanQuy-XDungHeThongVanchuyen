...
<head>
    ...
    <style>
        /* Tối thiểu: thêm vài style để có tab + card (không phá styles.css hiện tại) */
        .kh-container { max-width: 1100px; margin: 0 auto; padding: 24px 16px; }
        .kh-header { display:flex; align-items:center; justify-content:space-between; gap:12px; margin-bottom: 16px; }
        .kh-header h1 { margin:0; font-size: 22px; }
        ...
        .kh-btn.secondary { background: #eef2ff; color:#2d3748; border: 1px solid #dbeafe; }
        .kh-tabs { display:flex; gap:10px; flex-wrap:wrap; margin: 12px 0 16px; }
        .kh-tab {
        ...
        .kh-tab.active { background: #111827; border-color:#111827; color:#fff; }
        .kh-card { background:#fff; border: 1px solid #e5e7eb; border-radius: 14px; padding: 16px; }
        .kh-grid { display:grid; grid-template-columns: 1fr; gap: 14px; }
        @media (min-width: 900px) { .kh-grid.cols-2 { grid-template-columns: 1fr 1fr; } }

        .kh-field { display:grid; grid-template-columns: 160px 1fr; gap: 10px; padding: 8px 0; border-bottom: 1px dashed #e5e7eb; }
        .kh-field:last-child { border-bottom: 0; }
        .kh-field .label { color:#6b7280; }
        .kh-field .value { color:#111827; font-weight: 600; }

        .kh-table { width:100%; border-collapse: collapse; }
        .kh-table th, .kh-table td { padding: 10px; border-bottom: 1px solid #e5e7eb; text-align:left; }
        .kh-table th { background: #f9fafb; color:#374151; font-weight:700; }
        ...
        .kh-muted { color:#6b7280; }
        .kh-error { color:#b91c1c; background:#fef2f2; border:1px solid #fecaca; padding:10px; border-radius: 10px; }
        .kh-success { color:#065f46; background:#ecfdf5; border:1px solid #a7f3d0; padding:10px; border-radius: 10px; }

        .track-box { display:flex; gap:10px; flex-wrap:wrap; margin-top:10px; }
        .track-box input {
            flex:1; min-width: 220px;
            padding: 12px 12px; border-radius: 10px; border:1px solid #e5e7eb;
            outline:none;
        }
        ...
        .kh-profile-card {
            ...
            border: 1px solid #dbeafe; border-radius: 16px; padding: 20px 24px;
            max-width: 560px;
        }
        .kh-profile-avatar {
        ...
        .kh-profile-name { font-size: 20px; font-weight: 700; color: #111827; }
        .kh-profile-meta { font-size: 13px; color: #6b7280; margin-top: 2px; }
        .kh-profile-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-top: 16px; }
        @media (max-width: 520px) { .kh-profile-grid { grid-template-columns: 1fr; } }
        .kh-info-item { background: #fff; border: 1px solid #e5e7eb; border-radius: 10px; padding: 12px 14px; }
        .kh-info-item .lbl { font-size: 11px; color: #9ca3af; text-transform: uppercase; letter-spacing: .5px; margin-bottom: 4px; }
        .kh-info-item .val { font-size: 15px; font-weight: 600; color: #111827; word-break: break-all; }
        .kh-edit-btn {
            margin-top: 18px;
        ...
        .kh-input:focus { border-color: #667eea; }
        ...
        .od-step.active .od-step-label { color: #667eea; font-weight: 600; }
...
<body>
    <header class="header">
        <div class="nav-container">
            ...
            <div class="nav-links">
                ...
                <a href="../../backend/dangxuat.php" class="btn-login">Đăng xuất</a>
            </div>
        </div>
    </header>
    ...
    <div class="kh-container">
        ...
        <section id="tab-track" class="kh-card">
            <h2 style="margin-top:0;">Tra cứu đơn hàng</h2>
        ...
        <section id="tab-orders" class="kh-card" style="display:none;">
            <div style="display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap;">
                <h2 style="margin:0;">Đơn của tôi</h2>
                <button class="kh-btn secondary" id="btnReloadOrders" type="button">Tải lại</button>
            </div>
            <div class="kh-muted" style="margin-top:6px;">
                Danh sách đơn hàng mà bạn là người gửi hoặc người nhận (dựa theo số điện thoại đăng nhập).
            </div>

            <div id="ordersBox" style="margin-top:12px;">
                <div class="kh-muted">Đang tải...</div>
            </div>
        </section>
        ...
        <section id="tab-profile" class="kh-card" style="display:none;">
            ...
            <div id="editProfileSection" style="display:none;">
                <h3 style="margin-top:0; margin-bottom:16px; font-size:16px;">✏️ Chỉnh sửa thông tin</h3>
                <div id="editProfileMsg" style="display:none; margin-bottom:14px;"></div>
                <form id="editProfileForm" style="max-width:480px; display:grid; gap:14px;">
                    <div>
                    ...
                    <div>
                        ...
                        <small style="color:#9ca3af;">Số điện thoại không thể thay đổi</small>
                    </div>
                    <div>
                    ...
                    <div style="display:flex; gap:10px;">
                        <button class="kh-btn" type="submit" id="btnSaveProfile">💾 Lưu thay đổi</button>
            ...
            <!-- Form đổi mật khẩu -->
            <div>
                <h3 style="margin-top:0; margin-bottom:4px; font-size:16px;">🔒 Đổi mật khẩu</h3>
                <p class="kh-muted" style="margin-bottom:16px;">Nhập mật khẩu hiện tại để xác nhận danh tính.</p>
                <div id="changePwdMsg" style="display:none; margin-bottom:14px;"></div>
                <form id="changePwdForm" style="max-width:480px; display:grid; gap:14px;">
                    <div>
                    ...
                    <div id="pwdStrengthBar" style="display:none;">
                        ...
                        <small id="pwdStrengthLabel" style="color:#6b7280;"></small>
                    </div>
                    <div>
                        <button class="kh-btn" type="submit" id="btnChangePwd">🔑 Đổi mật khẩu</button>
                    </div>
                </form>
            </div>
        </section>
    ...
    <script>
        ...
        async function trackOrder(code) {
            ...
            try {
                const res = await fetch(`${API_BASE}?action=track&code=${encodeURIComponent(code)}`, {
                    method: 'GET'
                });
                    ...
                    <div class="kh-profile-card">
                        ...
                        <div class="kh-profile-grid">
                            <div class="kh-info-item">
                                ...
                                <div class="val">
                                    ...
                                    <button type="button" onclick="toggleReveal('sdtDisplay','${sdt}', this)"
                                        style="margin-left:8px; background:none; border:none; cursor:pointer; color:#667eea; font-size:13px;">Xem</button>
                                </div>
                            </div>
                            <div class="kh-info-item">
                            ...
                            <div class="kh-info-item">
                                ...
                                <div class="val">${trangThai}</div>
                            </div>
                            <div class="kh-info-item">
                                <div class="lbl">Địa chỉ</div>
                                <div class="val" style="font-size:13px;">${escapeHtml(p.dia_chi || '—')}</div>
                            </div>
                        </div>
        ...
        function toggleReveal(spanId, realVal, btn) {
            ...
            if (btn.textContent === 'Xem') {
                span.textContent = realVal;
                btn.textContent = 'Ẩn';
            } else {
                span.textContent = maskSensitive(realVal);
                btn.textContent = 'Xem';
            }
        }
        ...
        document.getElementById('changePwdForm').addEventListener('submit', async (e) => {
            ...
            if (newPwd !== confirmPwd) {
                msgBox.className = 'kh-error';
                msgBox.textContent = 'Xác nhận mật khẩu không khớp';
                msgBox.style.display = 'block';
                return;
            }
            ...
            try {
                ...
                const rows = orders.map(o => `
                    <tr>
                        ...
                        <td>
                            <button class="kh-btn secondary" type="button" onclick="showOrderDetail('${String(o.ma_don || '').replace(/'/g, "\\'")}')">📋 Chi tiết</button>
                        </td>
                    </tr>
                `).join('');
        ...
        document.addEventListener('keydown', e => {
            if (e.key === 'Escape') closeOrderDetailBtn();
        });
        ...
        async function showOrderDetail(madon) {
            ...
            try {
                ...
                const ng = o.nguoi_nhan || {};
                const gg = o.nguoi_gui  || {};
                const infoHtml = `
                    <div class="od-info">
                        <div class="od-info-box">
                            <div class="lbl">Người gửi</div>
                            <div class="val">${escapeHtml(gg.ho_ten || '—')}</div>
                            <div style="font-size:12px;color:#6b7280;">${escapeHtml(gg.so_dien_thoai || '')}</div>
                        </div>
                        <div class="od-info-box">
                            <div class="lbl">Người nhận</div>
                            <div class="val">${escapeHtml(ng.ho_ten || '—')}</div>
                            <div style="font-size:12px;color:#6b7280;">${escapeHtml(ng.so_dien_thoai || '')}</div>
                            <div style="font-size:12px;color:#6b7280;">${escapeHtml(ng.dia_chi || '')}</div>
                        </div>
                        <div class="od-info-box">
                            <div class="lbl">Khối lượng</div>
                            <div class="val">${escapeHtml(String(o.tong_khoi_luong_kg || o.khoi_luong_kg || '—'))} kg</div>
                        </div>
                        <div class="od-info-box">
                            <div class="lbl">Phí vận chuyển</div>
                            <div class="val">${formatCurrency(o.phi_van_chuyen)}</div>
                            ${remaining > 0 ? `<div style="font-size:12px;color:#e11d48;">Còn thu: ${formatCurrency(remaining)}</div>` : '<div style="font-size:12px;color:#22c55e;">Đã thanh toán đủ</div>'}
                        </div>
                        ${o.chi_nhanh_gui ? `<div class="od-info-box">
                            <div class="lbl">Chi nhánh gửi</div>
                            <div class="val" style="font-size:13px;">${escapeHtml(o.chi_nhanh_gui)}</div>
                        </div>` : ''}
                        ${o.chi_nhanh_nhan ? `<div class="od-info-box">
                            <div class="lbl">Chi nhánh nhận</div>
                            <div class="val" style="font-size:13px;">${escapeHtml(o.chi_nhanh_nhan)}</div>
                        </div>` : ''}
                    </div>
                `;

                // ── Timeline (mốc thời gian, mới nhất lên đầu) ──
                ...
                body.innerHTML = `
                    ...
                    <div style="margin-bottom:8px;">
                        <div style="display:flex; justify-content:space-between; font-size:12px; color:#6b7280; margin-bottom:4px;">
                            ...
                            <span>${isCancel ? '❌ Đã hủy' : `${o.progress||0}%`}</span>
                        </div>
                        <div style="background:#e5e7eb; border-radius:999px; height:6px; overflow:hidden;">
                            <div style="width:${isCancel?'100':o.progress||0}%; height:100%;
                                background:${isCancel ? '#ef4444' : 'linear-gradient(90deg,#667eea,#764ba2)'};
                    ...
                    <div style="margin-top:4px;">
                        <div style="font-weight:700; font-size:14px; margin-bottom:14px; color:#374151;">
                            🕐 Lịch sử vận chuyển
                        </div>
        ...
        // Preload profile in background (optional)
        // loadMyProfile();
    </script>
</body>
</html>