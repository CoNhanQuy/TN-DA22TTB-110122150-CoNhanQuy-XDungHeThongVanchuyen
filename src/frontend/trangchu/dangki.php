<?php
include_once '../../backend/config/cauhinh.php';
            ...
            if (!$error) {
                ...
                if (!$ins) {
                    ...
                    if ($ins->execute()) {
                        ...
                        // Kiểm tra xem số điện thoại đã tồn tại trong khach_hang chưa (do NV tiếp nhận tạo)
                        $stmtKh = $conn->prepare("SELECT id FROM khach_hang WHERE so_dien_thoai = ? LIMIT 1");
                        $stmtKh->bind_param("s", $sdt);
                        $stmtKh->execute();
                        $resKh = $stmtKh->get_result();
                        if ($resKh && $resKh->num_rows > 0) {
                            // Đã có, liên kết nguoi_dung_id
                            $rowKh = $resKh->fetch_assoc();
                            $khId = $rowKh['id'];
                            $updKh = $conn->prepare("UPDATE khach_hang SET nguoi_dung_id = ? WHERE id = ?");
                            $updKh->bind_param("ii", $newId, $khId);
                            $updKh->execute();
                            $updKh->close();
                        } else {
                            // Chưa có, thêm mới record vào khach_hang để hiển thị ở trang admin
                            $insKh = $conn->prepare("INSERT INTO khach_hang (nguoi_dung_id, ho_ten, so_dien_thoai, dia_chi) VALUES (?, ?, ?, '')");
                            $insKh->bind_param("iss", $newId, $ho_ten, $sdt);
                            $insKh->execute();
                            $insKh->close();
                        }
                        $stmtKh->close();
