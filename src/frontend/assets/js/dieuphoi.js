...
function getStatusClass(status) {
    const map = {
        'cho_khoi_hanh': 'status-waiting',
        'dang_di_chuyen': 'status-assigned',
        'da_den_kho_nhan': 'status-done',
...
function formatTrangThai(status) {
    const map = {
        'cho_khoi_hanh': 'Chưa khởi hành',
        'dang_di_chuyen': 'Đang chạy',
        'da_den_kho_nhan': 'Hoàn thành',
        'huy': 'Đã hủy'
    };
...
function isExpiredAndPending(shipment) {
    if (shipment.trang_thai !== 'cho_khoi_hanh') return false;
...
async function initPage() {
    try { await loadStats(); } catch(e) { console.error('Error loadStats:', e); }
    try { await loadPendingOrders(); } catch(e) { console.error('Error loadPendingOrders:', e); }
    try { await loadShipments(); } catch(e) { console.error('Error loadShipments:', e); }
    try { await loadDrivers(); } catch(e) { console.error('Error loadDrivers:', e); }
    try { await loadVehicles(); } catch(e) { console.error('Error loadVehicles:', e); }
    try { await loadRoutes(); } catch(e) { console.error('Error loadRoutes:', e); }
    try { await loadShippers(); } catch(e) { console.error('Error loadShippers:', e); }
    try { await loadShipperOrders(); } catch(e) { console.error('Error loadShipperOrders:', e); }
}
...
function loadShippers() {
    return apiFetch(`${getApiBase()}/backend/api/index.php?action=available_shippers`)
        ...
        .then(data => {
            ...
            if (data.success && data.data && data.data.shippers) {
                sel.innerHTML = '<option value="">-- Chọn shipper --</option>';
                sel.innerHTML += data.data.shippers.map(s => 
                    `<option value="${s.id}">${s.ho_ten} (${s.so_dien_thoai || ''})</option>`
                ).join('');
            }
        })
        .catch(err => console.error('Lỗi tải danh sách shipper:', err));
}
...
function loadShipperOrders() {
    return apiFetch(`${getApiBase()}/backend/api/index.php?action=orders_for_shipper`)
        ...
        .then(data => {
            ...
            if (data.success && data.data && data.data.length > 0) {
                allShipperOrders = data.data;
                tbody.innerHTML = data.data.map(order => {
                    const shipperName = order.assigned_shipper_name 
                        ? `<span style="color: #2b8a3e; font-weight: 600;">${order.assigned_shipper_name}</span>`
                        : '<span style="color: #ae3ec9; font-style: italic;">Chưa phân công</span>';
                ...
                allShipperOrders = [];
                tbody.innerHTML = '<tr><td colspan="7" style="text-align: center; color: #999;">Không có đơn hàng nào cần giao tận nơi</td></tr>';
            }
        })
        .catch(err => {
            console.error('Lỗi tải đơn hàng phân công shipper:', err);
            document.getElementById('shipperOrdersList').innerHTML = 
                '<tr><td colspan="7" style="text-align: center; color: #e03;">Lỗi tải dữ liệu</td></tr>';
        });
}

function toggleCheckAllShipper(masterCb) {
    document.querySelectorAll('.shipper-order-checkbox').forEach(cb => cb.checked = masterCb.checked);
}

function assignSelectedToShipper() {
    const shipperId = document.getElementById('shipperSelect').value;
    if (!shipperId) {
        alert('Vui lòng chọn shipper trước khi xác nhận phân công!');
        return;
    }

    const checkedBoxes = document.querySelectorAll('.shipper-order-checkbox:checked');
    ...
    if (!confirm(`Bạn có chắc chắn muốn phân công ${orderIds.length} đơn hàng đã chọn cho shipper này?`)) {
        return;
    }

    const formData = new FormData();
    formData.append('action', 'assign_to_shipper');
    ...
    .then(data => {
        if (data.success) {
            alert('✓ ' + data.message);
            loadShipperOrders();
            document.getElementById('checkAllShipper').checked = false;
        } else {
            alert('Lỗi: ' + (data.message || 'Không thể phân công đơn hàng'));
        }
    })
    .catch(err => {
        console.error('Lỗi phân công:', err);
        alert('Lỗi kết nối khi phân công đơn hàng.');
    });
}
